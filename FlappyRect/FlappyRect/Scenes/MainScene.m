...
- (void)addHeroNode
{
    self.hero = [SKSpriteNode spriteNodeWithImageNamed:@"bulle2.png"];
...
- (void)addWall
{
    ...
    CGFloat holeLength = HERO_SIZE.height * 6;
...
- (void)restart
{
    NSLog(@"Le current Point %d",currentPoint);
    
    if (currentPoint >= 10)
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"BIEN JOUÉ"
                                                        message:@"Bravo! MDP : «Vive les bulles»"
                                                       delegate:self
    ...
    NSLog(@"before restar");
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"FATALITY"
                                                        message:@""
                                                       delegate:self
                                              cancelButtonTitle:@"OK"
                                              otherButtonTitles:nil];
...
- (void)didBeginContact:(SKPhysicsContact *)contact
{
    ...
    if ((firstBody.categoryBitMask & heroCategory) && (secondBody.categoryBitMask & holeCategory)) {
        currentPoint = [_labelNode.text intValue];
